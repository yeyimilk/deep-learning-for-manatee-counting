...
function drawLineNew(context, x1, y1, x2, y2, borderColor = null, fillColor = null, borderWidth = 1) {
    context.beginPath();
    context.lineCap = 'round';
...
Box.prototype.draw = function (rectify = false) {
    ...
    drawLineNew(ctx, this.sx, this.sy, this.ex, this.ey, labelLineColor, null, labelLineWidth)
    if (rectify) {
        ctx.save();
...
Box.prototype.distance = function(x, y, returnIndex=false) {
    ...
    // else return minval;
    var a = this.ey - this.sy
    var b = this.sx - this.ex
    var c = this.ex * this.sy - this.sx * this.ey
    // ax + by + c
    var dis = Math.abs(a * x + b * y + c) / Math.sqrt(a^2 + b^2)
    return dis
}
...
// function Line(sx = 0, sy = 0, ex = 0, ey = 0) {
//     this.sx = sx;
//     this.sy = sy;
//     this.ex = ex;
//     this.ey = ey;
...
//     return new Line(this.sx, this.sy, this.ex, this.ey);
// }
// Line.prototype.cloneFrom = function(_line) {
//     this.sx = _line.sx;
//     this.sy = _line.sy;
//     this.ex = _line.ex;
//     this.ey = _line.ey;
...
// Line.prototype.draw = function (rectify = false) {
//     drawLineNew(ctx, this.sx, this.sy, this.ex, this.ey, labelLineColor, labelLineWidth)
//     if (rectify) {
//         ctx.save();
//         ctx.setLineDash([5, 10]);
//         drawRect(ctx, sx, sy, ex, ey, '#fff', null, labelRectifyWidth);
//         ctx.restore();
//     }
// }
...
// // have no idea
// Line.prototype.nearby = function (x, y) {
//     var dis = this.distance(x, y);
//     return dis < labelLineWidth ? 5 : 0;
// }
// // have no idea
// Line.prototype.normalize = function (ix, iy, iw, ih) {
//     var line = new Line(
//         (this.sx - ix) / iw,
//         (this.sy - iy) / ih,
//         (this.ex - ix) / iw,
...
// }
// // have no idea
// Line.prototype.unnormalize = function (ix, iy, iw, ih) {
//     var line = new Line(
//         (this.sx * iw) / ix,
//         (this.sy * ih) / iy,
//         (this.ex * iw) / ix,
